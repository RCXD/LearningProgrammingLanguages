...
void Enqueue(Q* q) {
	...
	printf("\n");
	printf("어떤 영화를 보시겠습니까? ");

	//영화 번호 입력
	int num;//입력한것보다 1 적은 인덱스에 접근해야 맞음
	scanf("%d", &num);
	num--; //주소, 포인터...일때...?
	//node->data->movie = MT[num]; //안됨.
	strcpy(node->data.movie, MT[num]);
	//strcpy(A,B); << B의 문자열값을 A의 문자열공간에 저장하는 함수
	printf("예약자 성함입력: ");
	scanf("%s", node->data.name); //문자열 입력시 & 안씀.
	node->next = NULL;
	//여기까지 노드생성 완료
...
void Dequeue(Q* q) {
	...
	if (n == q->front->data.data) {
		//입장
		printf("%s 즐거운 관람되세요~!!\n", q->front->data.movie);
		q->front = q->front->next; //맞나?
	}
	else {
		//roll() 구현
			//kim -> lee -> park ==> lee -> park -> kim //삭제하는게 아닌, 뒤로 밀어줌
		q->rear->next = q->front;
