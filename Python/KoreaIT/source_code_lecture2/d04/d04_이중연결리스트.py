...
class DoubleLinkedList:
    ...
    def indexOf(self, data): 
        #가운데에 가까우면 운좋으면 검색이 빨리 끝난다고.
        #일단 여기서는 1번부터 이동한다고.
        curr=self.head
        for i in range(self.count):
            curr=curr.next #헤드 다음거 연결
            if curr.data == data: #받은 데이터가 curr의 데이터와 같은지 확인
                return i+1 #위치(인덱스) 반환
                #헤드의 인덱스는 0번이니, 저장해놓은 A가 1이 나오려면
                #for문이 한번 더 돌면 됨.
                #복습...필요
